...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            cursor.execute('INSERT INTO usuario (username, password, chat) VALUES (%s, %s, %s)',
        ...
        if request.method == 'POST' and 'username' in request.form and 'password' in request.form  and 'chat' in request.form: 
            ...
            if account: 
                if not re.match(r'[A-Za-z0-9]+', username): 
                    ...
                    cursor.execute('UPDATE usuario SET username = %s, password = %s, chat = %s WHERE id = %s', (username, password, chat, session['id']))
                    mysql.connection.commit() 
